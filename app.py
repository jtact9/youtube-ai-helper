...
st.markdown("""
    ...
    .stButton>button { 
        ...
        background-color: #E74C3C; color: white !important; 
        font-weight: bold; border: none; 
    }

    /* 공지 카드 스타일 (라이트 모드에서도 가독성 유지) */
    .notice-card-pinned { 
        background-color: #2d333b; padding: 20px; border-radius: 10px; 
        border-left: 8px solid #f1c40f; margin-bottom: 15px; border-top: 1px solid #f1c40f; 
        color: #FFFFFF !important;
    }
    .notice-card { 
        background-color: #21262d; padding: 20px; border-radius: 10px; 
        border-left: 5px solid #E74C3C; margin-bottom: 15px; 
        color: #FFFFFF !important;
    }
    ...
    .tag-box { 
        ...
        font-family: monospace; font-size: 1.1rem; line-height: 1.8; 
    }
    .big-font { font-size: 1.4rem !important; font-weight: 700; color: #FFFFFF !important; margin-bottom: 15px; display: block; }
...
def load_notices():
    default_notice = [{"id": 123456, "date": datetime.now().strftime("%Y-%m-%d"), "tag": "필독", "content": "🚨 [가이드] 생성 중 메뉴 이동 시 작업이 초기화됩니다. 에러 발생 시 엔진 변경 후 1분 뒤 재시도 바랍니다. 🚨", "image": None, "pinned": True}]
...
try:
    ...
    st.error("⚠️ Secrets 설정 오류 (GEMINI_API_KEY 확인 필요)")
    st.stop()
...
# [기타 비즈니스 격식 변환기 및 콘텐츠 기획 콘티 로직 v8.2와 동일 유지]
